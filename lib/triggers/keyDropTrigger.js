...
KeyDropTrigger.prototype._onLoad = async function(){
	var that = this;
...
KeyDropTrigger.prototype._refreshFile = async function(){ //move into its own function for reuse (this way can edit file manually)
	var that = this;
	try {
		that.keys = JSON.parse(await readFileAsync(that.options.dbFile));
...
KeyDropTrigger.prototype._startServer = async function(){
	if(this.chatBot.options.disableWebServer || this.serverStarted) {
	...
	var that = this;
	try {
		this.options.secret = JSON.parse(await readFileAsync(this.options.secretFile));
	} catch(err) {
		this.options.secret = require('crypto').randomBytes(64).toString('hex');
		fsp.ws(this.options.secretFile,this.options.secret);
	}
	try {
		this.apikeys = JSON.parse(await readFileAsync(this.options.apikeyStore));
	} catch(err) {
		this.apikeys = {};
		fsp.ws(this.options.apikeyStore,JSON.stringify(this.apikeys));
	}
	var fslogfunc = async function(log) {
		try {
			that.winston.info(this.logPrefix+log);
		} catch(err) {
			console.log(err.stack);
			await writeFileAsync('error-keydroptrigger-'+Date(),err.stack); //why is this here? I can't remember...
		}
	}
	var fsopts = {
