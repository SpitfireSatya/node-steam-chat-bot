(async function() {
	...
	if (fs.existsSync(serversFile)) {
		try {
			...
			steam.servers = JSON.parse(await readFileAsync(serversFile));
		} catch(err) {
	...
	var ChatBot = function(username, password, options) {
		...
		if(!fs.existsSync(this.name+"/firstRun")) {
			...
			console.error(c.white.bgBlack('=================='));
			await writeFileAsync(this.name+"/firstRun","The presence of this file indicates that you received a message regarding the new default location of bot files.");
			process.exit();
		}
	...
	*/
		...
		this.steamClient.on("servers", async function(servers) {
			try{
				that.winston.silly(that.name+"/chatBot.js: Attempting to write servers file");
				await writeFileAsync(serversFile, JSON.stringify(servers));
			} catch(err) {
	...
	ChatBot.prototype.logOn = async function() {
		...
		if (!this.connected) {
			...
			try {
				var sha = '';
				if(fs.existsSync(this.sentryFile)) {
					var file = await readFileAsync(this.sentryFile);
	...
	////
	ChatBot.prototype._autojoinChatrooms = async function() {
		// Auto-join chat rooms that the bot was previously invited to (and not removed from)
		if (fs.existsSync(this.autojoinFile)) {
			this.autojoinRooms = JSON.parse(await readFileAsync(this.autojoinFile));
			var that = this;
	...
	ChatBot.prototype._addChatToAutojoin = async function(roomId) {
		if (fs.existsSync(this.autojoinFile)) {
			this.autojoinRooms = JSON.parse(await readFileAsync(this.autojoinFile));
		}
		else {
		...
		await writeFileAsync(this.autojoinFile, JSON.stringify(this.autojoinRooms));
	}

	ChatBot.prototype._removeChatFromAutojoin = async function(roomId) {
		if (fs.existsSync(this.autojoinFile)) {
			this.autojoinRooms = JSON.parse(await readFileAsync(this.autojoinFile));
	...
	ChatBot.prototype._onUpdateMachineAuth = async function(sentry, cb) {
			this.winston.info(this.name+"/chatBot.js: Obtained sentry " + sentry.filename + ". Writing to " + this.sentryFile);
			await writeFileAsync(this.sentryFile, sentry.bytes);
			cb({
